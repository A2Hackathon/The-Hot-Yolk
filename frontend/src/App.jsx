  ...
  const generateWorld = async (promptText) => {
    ...
    try {
      ...
      if (data.world && data.world.lighting_config) {
        ...
        // UPDATED: Always create northern lights for arctic biomes
        const biomeName = data.world?.biome || data.world?.biome_name;
        const isArctic = biomeName && biomeName.toLowerCase() === 'arctic';
        if (isArctic || data.world.lighting_config.northern_lights) {
          console.log('[NORTHERN LIGHTS] Creating northern lights for arctic biome (visible day and night)');
  ...
  const modifyWorld = async (commandText) => {
    ...
    try {
      ...
      if (data.world?.lighting_config) {
        ...
        // Update northern lights based on lighting config flag
        const hasLights = scene.children.some(c => c.userData?.isNorthernLights);
        const shouldHaveLights = data.world.lighting_config.northern_lights === true;
        
        if (shouldHaveLights && !hasLights) {
          console.log('[MODIFY] Adding northern lights');
          createNorthernLights(scene);
        } else if (!shouldHaveLights && hasLights) {
          // Remove northern lights if flag is false
          console.log('[MODIFY] Removing northern lights');
          const lights = scene.children.filter(c => c.userData?.isNorthernLights);
